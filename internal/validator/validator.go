...
// this patter once at startup and storing the compiled *regexp.Regexp in a
...
// Valid() retrurns true if the FieldErrors map doesn't contain any entries.
...
// AddFieldError() adds an error message to the FieldErrors map (so long no
...
// ChecField() adds an error message to the FieldErrors map only if a
