...
func TestHumanDate(t *testing.T) {
	...
	for _, tt := range tests {
		...
		// indentify the sub-test in any log output) and the second parameter is
		// and anonumous function containing the actual test for each case.
		t.Run(tt.name, func(t *testing.T) {
			hd := humanDate(tt.tm)

			// Use the new assert.Equal() helper to compare and
			// actual values.
